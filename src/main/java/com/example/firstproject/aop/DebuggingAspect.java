...
@Aspect //AOP 클래스 선언->부가 기능을 주입하는 클래스이다.
@Component //IoC 컨테이너가 해당 객체를 생성 및 관리한다.
@Slf4j //로깅
public class DebuggingAspect {

    //대상 메소드 선택: CommentService의 create() 메소드, create(..): 어떤 파라미터가 들어가도 ok
    //@Pointcut("execution(* com.example.firstproject.service.CommentService.create(..))")
    //대상 범위 확장하기: CommentService의 모든 메소드
    ...
    //부가 기능 실행 시점 설정: cut()의 대상이 수행되기 이전
    @Before("cut()")
    public void loggingArgs(JoinPoint joinPoint){ //cut()의 대상 메소드
        ...
        for(Object obj : args){ //foreach문->향상된 for문
            log.info("{}#{}의 입력값 => {}", className, methodName, obj);
        }
    }

    //부가 기능 실행 시점 설정: cut()에 지정된 대상 호출 성공 후
